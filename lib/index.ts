...
export class Lib {
  ...
  getCount = (env = this.env) => {
    return env.client.query("lunctron-wars", { get_count: {} })
  }

  increment = (env = this.env) : Promise<WaitTxBroadcastResult> => {
    return env.client.execute("lunctron-wars", env.wallets.validator, { increment: {} })
  }
  
  reset = (env = this.env, count: number ) : Promise<WaitTxBroadcastResult> => {
    return env.client.execute("lunctron-wars", env.wallets.validator, { reset: { count } })
  }
};
